...
\begin{document}
  ...
  \ResumeEntryStart
    ...
    \ResumeItemListStart
      ...
      \ResumeItem{Chatbot for Querying Data}
        {Engineered a \textbf{Python}-based chatbot leveraging large language models to query data and generate charts from a \textbf{MariaDB} database through natural language prompts.}
    \ResumeItemListEnd
  \ResumeEntryEnd
  ...
  \ResumeEntryStart
    ...
    \ResumeItem{Consolidated Search API}
      {Developed a \textbf{Golang} API to aggregate and consolidate data from multiple databases into a single, unified API call, simplifying the information search process for 96,000+ users.}
    \ResumeItem{Market-Pool Linkage}
      {Used \textbf{Golang} to automatically link markets and liquidity pools, eliminating manual work and and securely allocating all USD \$130+ million of capital across the exchange.}
    ...
    \ResumeItem{Telegram Bot for Blockchain Monitoring}
      {Built a Telegram Bot using \textbf{TypeScript} and \textbf{Redis} to alert the team about stalled Carbon blockchain transactions, increasing the resolution speed by 83\%.}
    \ResumeItemListEnd
  \ResumeEntryEnd
