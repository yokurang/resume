...
\begin{document}
  ...
  \ResumeEntryStart
    ...
    \ResumeItemListStart
      \ResumeItemDefault{Will trade stock options on HKEX, researching and implementing high-frequency and mid-frequency strategies.}
    \ResumeItemListEnd
  \ResumeEntryEnd

  \ResumeEntryStart
    \ResumeEntryTSDL{Caladan}{HFT Market Making}{Software Engineer}{Jul 2025 – Present}
    \ResumeItemListStart
      \ResumeItemDefault{Built generalised caching layer for time-series datasets, reducing query latency from seconds to milliseconds.}
      \ResumeItemDefault{Re-engineered distributed ETL workflows using DAG modeling with fault-tolerant retries for high-availability.}
      \ResumeItemDefault{Developed real-time PnL computation service with streaming validation and metric exposure to Grafana.}
      \ResumeItemDefault{Created modular Python services for fills and market research with generalised architecture for cross-team reuse.}
    \ResumeItemListEnd
\ResumeEntryEnd
...
\ResumeEntryStart
  ...
  % \ResumeItemDefault{Enhanced C\# trading algorithm logic and resolved critical bugs, improving accuracy and reducing latency.}
...
\ResumeEntryEnd
  ...
  \ResumeEntryStart
    ...
    % \ResumeItemDefault{Implemented a Python RAG tool to generate trading ideas tailored to client portfolio goals and mandates.}  
    ...
    % \ResumeItem{Chatbot for Querying Data} {Engineered a \textbf{Python}-based chatbot leveraging large language models for querying data and generating charts from a \textbf{MariaDB} database through natural language prompts.}
    \ResumeItemListEnd
  \ResumeEntryEnd
  ...
  \ResumeEntryStart
    ...
    \ResumeItemListStart
      ...
      % \ResumeItemDefault{Designed and developed a comprehensive employee leave system using Vue.js, Laravel, and MySQL from scratch.}
  ...
  %   \ResumeItemDefault{Created a unified Golang search API to aggregate and consolidate data from multiple databases into one call.}
  %   \ResumeItemDefault{Developed a TypeScript service for monitoring transactions and visualizing trades and statistics with \textbf{React}.}
  ...
  \SectionHeading{Open Source}
    ...
    \ResumeItemListStart
      ...
      % \ResumeItemDefault{Implemented pager integration and terminal extensions (extunix) to handle streaming and navigation of very large diff files securely and reliably.}
    \ResumeItemListEnd
  \ResumeEntryEnd
  ...
  \ProjectItemListStart
    ...
    {Engineered ETL and ML pipeline to scrape Path of Exile item prices and estimate FX via VWAP. Researched features and built regularized regression and closed-form models with residual diagnostics. Deployed live client to detect undervalued items across multiple currencies in real time.}
